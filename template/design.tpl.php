...
<html>
<head>
<title>设计-天桥底-博客</title>
<meta name="Keywords" content="设计,天桥底,博客,轻博客,独立博客,社区">
...
<div class="ar_list_main">
	<div class="posts_wrap">
		...
		<div class="paged">
			...
			//显示分页
			if ($pagenum > 1) {
				$prev = ($page != 1)? "<a class='prev' href='index.php?do=article&ac=design&page=". ($page-1) . "'>上一页</a>" : "";
				echo $prev;
				for ($i = 1; $i <= $pagenum; $i++){
					$show = ($i != $page)? "<a href='index.php?do=article&ac=design&page=". $i . "'>$i</a>" : "<b>$i</b>";
					echo $show;
				}
				$next = ($page != $pagenum)? "<a class='next' href='index.php?do=article&ac=design&page=". ($page+1) . "'>下一页</a>" : "";
				echo $next;
			}
			?>
		</div>
